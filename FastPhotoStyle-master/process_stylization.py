...
def memory_limit_image_resize(con_img):
    ...
    if min(con_img.width,con_img.height) < MINSIZE:
    ...
    if max(con_img.width,con_img.height) > MAXSIZE:
        if con_img.width > con_img.height:
...
def stylization(stylization_module, smoothing_module, content_image_path, style_image_path, content_seg_path, style_seg_path, output_image_path,
                cuda, save_intermediate, no_post, label_weight_list, cont_seg_remapping=None, styl_seg_remapping=None):
    # Load image
    with torch.no_grad():
        ...
        cont_img = transforms.ToTensor()(cont_img).unsqueeze(0) # 1차원 추가
        ...
        if save_intermediate:
            with Timer("Elapsed time in stylization: %f"):
                stylized_img = stylization_module.transform(cont_img, styl_img, cont_seg, styl_seg,label_weight_list)
